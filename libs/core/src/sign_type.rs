...
/// Represents the configuration information for a particular model of sign.
///
...
impl SignType {
    ...
    /// # Errors
    ///
    /// Returns [`Error`]`(`[`ErrorKind::Argument`]`, _)` if the data is not 16 bytes long
    /// or does not correspond to a known sign type.
    ///
    /// # Examples
    ///
    /// ```
    ...
    pub fn from_bytes(bytes: &[u8]) -> errors::Result<Self> {
        if bytes.len() != 16 {
            bail!(ErrorKind::Argument("Expected 16-byte configuration data".to_owned()));
        }
        ...
        match (bytes[0], bytes[1]) {
            ...
            _ => bail!(ErrorKind::Argument(format!("Unknown sign configuration data {:?}", bytes))),
        }
    }
...
mod tests {
    ...
    fn verify_roundtrip(sign_type: SignType, expected_bytes: &[u8]) {
        let encoded = sign_type.to_bytes();
        assert_eq!(expected_bytes, encoded);

        let decoded = SignType::from_bytes(encoded).unwrap();
        assert_eq!(sign_type, decoded);
    }

    #[test]
    fn sign_type_roundtrip() {
        verify_roundtrip(
            SignType::Max3000Front112x16,
            &vec![0x04, 0x47, 0x00, 0x0F, 0x10, 0x1C, 0x1C, 0x1C, 0x1C, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
        );

        verify_roundtrip(
            SignType::Max3000Front98x16,
            &vec![0x04, 0x4D, 0x00, 0x0D, 0x10, 0x0E, 0x1C, 0x1C, 0x1C, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
        );

        verify_roundtrip(
            SignType::Max3000Side90x7,
            &vec![0x04, 0x20, 0x00, 0x06, 0x07, 0x1E, 0x1E, 0x1E, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
        );

        verify_roundtrip(
            SignType::Max3000Rear23x10,
            &vec![0x04, 0x61, 0x00, 0x04, 0x0A, 0x17, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
        );

        verify_roundtrip(
            SignType::Max3000Rear30x10,
            &vec![0x04, 0x62, 0x00, 0x04, 0x0A, 0x1E, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
        );

        verify_roundtrip(
            SignType::Max3000Dash30x7,
            &vec![0x04, 0x26, 0x00, 0x03, 0x07, 0x1E, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
        );

        verify_roundtrip(
            SignType::HorizonFront160x16,
            &vec![0x08, 0xB1, 0x00, 0x15, 0x0C, 0x10, 0x00, 0xA0, 0x04, 0x00, 0x28, 0x00, 0x00, 0x00, 0x00, 0x00],
        );

        verify_roundtrip(
            SignType::HorizonFront140x16,
            &vec![0x08, 0xB2, 0x00, 0x12, 0x04, 0x10, 0x00, 0x8C, 0x01, 0x03, 0x14, 0x28, 0x00, 0x00, 0x00, 0x00],
        );

        verify_roundtrip(
            SignType::HorizonSide96x8,
            &vec![0x08, 0xB4, 0x00, 0x07, 0x0C, 0x08, 0x00, 0x60, 0x02, 0x00, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00],
        );

        verify_roundtrip(
            SignType::HorizonRear48x16,
            &vec![0x08, 0xB5, 0x00, 0x07, 0x0C, 0x10, 0x00, 0x30, 0x01, 0x00, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00],
        );

        verify_roundtrip(
            SignType::HorizonDash40x12,
            &vec![0x08, 0xB9, 0x00, 0x06, 0x8C, 0x0C, 0x00, 0x28, 0x01, 0x00, 0x28, 0x00, 0x04, 0x00, 0x00, 0x00],
        );
    }
    ...
    fn unknown_type_rejected() {
        let data = vec![0x10, 0xB9, 0x00, 0x06, 0x8C, 0x0C, 0x00, 0x28, 0x01, 0x00, 0x28, 0x00, 0x04, 0x00, 0x00, 0x00];
        let sign_type = SignType::from_bytes(&data);
        assert_matches!(sign_type, Err(Error(ErrorKind::Argument(_), _)));
    }

    #[test]
    fn unknown_horizon_rejected() {
        let data = vec![0x08, 0xBA, 0x00, 0x06, 0x8C, 0x0C, 0x00, 0x18, 0x01, 0x00, 0x28, 0x00, 0x04, 0x00, 0x00, 0x00];
        let sign_type = SignType::from_bytes(&data);
        assert_matches!(sign_type, Err(Error(ErrorKind::Argument(_), _)));
    }
    ...
    fn unknown_max3000_rejected() {
        let data = vec![0x04, 0x21, 0x00, 0x06, 0x07, 0x10, 0x10, 0x10, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
        let sign_type = SignType::from_bytes(&data);
        assert_matches!(sign_type, Err(Error(ErrorKind::Argument(_), _)));
    }
    ...
    #[test]
    fn too_much_data() {
        let data = vec![0x08, 0xB9, 0x00, 0x06, 0x8C, 0x0C, 0x00, 0x28, 0x01, 0x00, 0x28, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00];
        let sign_type = SignType::from_bytes(&data);
        assert_matches!(sign_type, Err(Error(ErrorKind::Argument(_), _)));
    }
}
