...
impl<'a> Page<'a> {
    ...
    /// # Errors
    ///
    /// Returns [`Error`]`(`[`ErrorKind::Argument`]`, _)` if the data length does not match
...
mod tests {
    ...
    fn wrong_size_rejected() {
        let page = Page::from_bytes(90, 7, vec![0x01, 0x01, 0x03]);
        assert_matches!(page, Err(Error(ErrorKind::Argument(_), _)));
    }
